...
- `bootstrap/` — one-time root config that creates the S3+DynamoDB remote state backend other
  root configs depend on. Uses local state itself (see its README for why).
...
Each module's Inputs/Outputs tables live between `<!-- BEGIN_TF_DOCS -->`/`<!-- END_TF_DOCS -->`
markers and are generated by terraform-docs, not hand-written. **Never hand-edit a table between
those markers** — if you add/change a variable or output, either run
`terraform-docs --config .terraform-docs.yml <module-dir>` yourself, or tell the user to run
`pre-commit run terraform_docs --all-files`, so the generated content and the actual
